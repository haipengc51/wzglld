...
public class RecordDeviceApplyAdapter extends MyBaseAdapter {

    public RecordDeviceApplyAdapter(Context context, List dataList) {
        super(context, dataList);
    }

    @Override
    public View createCellView(ViewGroup parent) {
        return mInflater.inflate(R.layout.adapter_record_device_apply_detail, parent, false);
    }
    ...
    public View buildData(int position, View cellView, BusinessHolder viewHolder) {
        ...
        DeviceapplyEntity item = (DeviceapplyEntity) dataList.get(position);
        myViewHolder.operatorPeople.setText(CommonUtils.getDataIfNull(item.getCzrname()));
        myViewHolder.operatorTime.setText(TimeUtils.dateToStringYYYYmmddHHMMSS(item.getSQSJ()));
